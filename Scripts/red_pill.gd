...
func _physics_process(delta: float) -> void:
	...
	if randi_range(0,5) == 3 and (
			is_on_floor() or (isBird and position.y < birdHeight)) and not outside:
...
func _on_area_3d_body_entered(_body: Node3D) -> void:
	...
	Globals.playerRef.takeKnockback(knockbackMultiplier, "%s was hit by that " + enemyName, 
		Globals.arrayHasAny(Globals.knockbacklink_sources, ["Any", "Enemies", enemyName]))
